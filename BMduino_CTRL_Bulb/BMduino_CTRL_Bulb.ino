#include "commlib.h"
#include "RGBLEDLib.h"  // RGBLED 自訂模組，用於控制 RGB 燈泡
#include "BT5Lib.h"
/*
...
{
  // 讀取 BLE 狀態與接收數據(透傳模式)
  // receiveBuf 為接收資料的緩衝區，receiveLen 為接收到的資料長度
  ...
  if (Status) // 若有狀態事件發生
  {
    switch (Status)
    {
      case API_CONNECTED: // BLE 已連線
        ...
        {
          board_connect = true;          // 標記已連線
          board_receive = false;         // 重置接收旗標
          if (board_conIntv == false)    // 連線成功後需設定一次連線間隔
          {
            BC7701.wakeUp();              // 喚醒藍牙模組
            delay(30);
            // 設定連線間隔參數
            // CON_MIN、CON_MAX 為最小/最大連線間隔（單位：1.25ms）
            // CON_LATENCY 為從設備延遲次數，CON_TIMEOUT 為連線超時時間
