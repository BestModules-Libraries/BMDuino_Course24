/**********************************************************************************************
  程式名稱：血氧系統語音播放系統
  設計目的：IoT 溫溼度 MQTT 雲端監控 + OLED 顯示 + 語音播報整合系統
...
{              
  ...
  if (ReciveBufflen)   // 若收到的資料長度不為 0，表示有有效訊息
  {
    ...
    int OXValue       = doc["Oxygen"]; // 取得血氧值，例如 95
    int HBValue       = doc["Heartbeat"];    // 取得心跳值，例如87
...
*/
    ...
    Serial.print("Oxygen: ");
    Serial.println(OXValue);
    Serial.print("Heartbeat: ");
    Serial.println(HBValue);

    // -------------------------------
    // 顯示於 OLED 螢幕
    // -------------------------------
  showMsgonOled("Spo2:"+String(OXValue)+"%"+"H:"+String(HBValue)+"BPM",4);           // 顯示裝置 ID

    // -------------------------------
    // 語音播報區 (需語音模組支援)
    // -------------------------------
    sayTitle(VOC_13);                          // 播放語音提示音「資料公告開始」
    saySensor(VOC_22, String(OXValue), VOC_17); // 播放血氧數值（如：「目前溫度 27 度」）
    saySensor(VOC_23, String(HBValue), VOC_32); // 播放心跳數值（如：「目前濕度 55%」）
  }
}
