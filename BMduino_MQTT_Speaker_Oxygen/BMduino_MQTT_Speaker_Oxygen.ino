...
{              
  ...
  if (ReciveBufflen)   // 若收到的資料長度不為 0，表示有有效訊息
  {
    ...
    int OXValue       = doc["Oxygen"]; // 取得溫度值，例如 27.5
    int HBValue       = doc["Heartbeat"];    // 取得濕度值，例如 55.3
...
*/
    ...
    saySensor(VOC_22, String(OXValue), VOC_17); // 播放溫度數值（如：「目前溫度 27 度」）
    saySensor(VOC_23, String(HBValue), VOC_11); // 播放濕度數值（如：「目前濕度 55%」）
  }
}
