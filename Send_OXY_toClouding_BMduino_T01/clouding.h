...
void SendtoClouding()     //傳送感測資料到雲端
{
   oxyvalue = random(85, 99) ;  // random(190,281) 產生 190~280 的整數，除以10.0得到小數
  // 產生濕度（60 ~ 99）的隨機值
  hbvalue = random(70, 110) ;
 ...
 if  (Wifi.getStatus())
 {
    Wifi.http_begin(ServerURL,ServerPort,connectstr);//begin http get 
      Serial.println("1:"+ServerURL);
      Serial.println("2:"+connectstr);
    Wifi.http_get();//http get opration
   webresponse = Wifi.http_getString();//get http result
    Serial.println("response:"+webresponse);//get http result
