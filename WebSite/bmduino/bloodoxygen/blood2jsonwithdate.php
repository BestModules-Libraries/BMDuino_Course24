...
// http://iot.arduino.org.tw:8888/bigdata/bloodoxygen/blood2jsonwithdate.php?MAC=48E729732158&start=20200406&end=20251231
// http://localhost:8888/bigdata/bloodoxygen/blood2jsonwithdate.php?MAC=48E729732158&start=20200406&end=20251231
...
// 3. 按 systime 升冪排序
// ---------------------------------------------------
$qry1 = "select * from bloodoxygen where MAC = '%s' and systime >= '%s' and systime <= '%s' order by systime asc";
...
if ($count > 0) {
  ...
  $d2 = array();  // 儲存心跳值 (heartbeat)
  $d3 = array();  // 儲存時間 (systime 的部分字串)

  // 逐筆讀取資料
  while($row = mysqli_fetch_array($result)) {
     // 轉型後丟到對應陣列
     array_push($d1, (float)$row["blood"]);          // 血氧值轉 float
     array_push($d2, (int)$row["heartbeat"]);        // 心跳值轉 int
     array_push($d3, substr($row["systime"],4,9));   // systime 從第5字元開始取9位數 → 縮短時間字串
  }

  // 將查詢結果整理到物件裡 
  $maindata->Device = $sid;
  $maindata->Interval = $s1."~".$s2;
  $maindata->DateTime = $d3;
  $maindata->Oxygen = $d1;
  $maindata->HeartBeat = $d2;
