...
// 表格標題，包含 MAC 地址
 $tstr = "IR Temperature Device List for MAC %s(紅外線體溫偵測裝置(ID:%s) 共 %d 筆" ;

// 定義表格行的模板，用於顯示資料
$subrow = "<tr><td>%s</td><td>%s</td><td>%s</td><td>%4.2f</td></tr>";

// SQL 查詢字串，使用給定的 MAC 地址
$str = "SELECT * FROM bodytemp WHERE MAC = '%s' order by systime desc ";
...
if ($result !== FALSE) 
{
    ...
    while ($row = mysqli_fetch_array($result)) 
    {
        ...
        array_push($d04, $row["bodytemperature"]); // 測距裝置距離值
    }
} else  
 {
     $countrec = 0 ;
...
<head>
    ...
    <title><?php echo sprintf($tstr,$temp0,$temp0,$countrec); ?> </title>
    ...
    <div align="center">
        <table border="1" align="center" cellspacing="1" cellpadding="1">
            ...
            <tr bgcolor="#CFC">
                <td colspan="5">
                    <div align="center">
                        <?php echo sprintf($tstr, $temp0, $temp0,$countrec); ?> <!-- 表格標題，包含 MAC 地址 -->
                    </div>
                </td>
            </tr>
            <!-- 表格列標題 -->
