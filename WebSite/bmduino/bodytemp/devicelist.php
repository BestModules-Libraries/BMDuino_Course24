<?php
/*
 這段程式碼顯示了一個頁面，列出了體溫感測器的 MAC 地址、溫度、最後更新時間等信息。
 * SQL 查詢從 big.bodytemp 資料表中獲取數據，
...
// 格式化表格每一行的模板
$subrow = "<tr><td>%s</td><td>%s</td><td>%4.2f</td><td>%d</td><td>%s</td></tr>";

// 格式化超連結的模板，用於查詢特定 MAC 地址的詳細資料
$op1 = "<a href='listforMAC.php?MAC=%s'>Device Detail(詳細每一筆資料)</a>";

// SQL 查詢字串，用於按 MAC 地址分組並查詢距離值和其他資訊
$qrystr = "SELECT MAC, count(*) as cnt, max(systime) as max , min(systime) as min FROM `bodytemp` WHERE 1 group by MAC order by MAC ASC;";
...
if ($result !== FALSE) { // 如果查詢成功，開始處理結果
    ...
    while ($row = mysqli_fetch_array($result))
    {
        ...
        array_push($d02, $row["min"]);
        array_push($d03, $row["max"]);
...
<head>
    ...
    <title>IR Temperature Device List(紅外線體溫偵測裝置)</title>
...
<body>
    ...
    <div align="center"> <!-- 居中對齊 -->
        ...
        <tr bgcolor="#CFC"> <!-- 標題行，帶有背景色 -->
            <td colspan='5'><div align='center'>IR Temperature Device List(紅外線體溫偵測裝置)</div></td> <!-- 合併六個單元格 -->
        </tr>
        <tr> <!-- 表格的標題欄 -->
            <td>MAC Address(網卡編號)</td>
            <td>Data Start Datetime(資料起始時間)</td>
            <td>Data End Datetime(資料結束時間)</td>
