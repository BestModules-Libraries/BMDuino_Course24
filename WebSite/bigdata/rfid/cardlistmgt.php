...
//$diagstr = "<div id='confirmDialog' style='display:none; position:fixed; top:30%; left:40%; padding:20px; background:#fff; border:1px solid #ccc; box-shadow:0 0 10px #aaa;'><p id='confirmMessage'>您是否要刪除第 %d 筆資料？</p><button onclick='proceedDelete()'>是</button><button onclick='closeDialog()'>不是</button></div>";

$admstr = "<a href='rfidformadd.php'>新增</a>、<a href='rfidformedt.php?id=%d'>修改</a>、<a href='#' onclick='confirmDelete(%d)'>刪除</a>" ;
...
<div id="confirmDialog" style="display:none; position:fixed; top:30%; left:40%; padding:20px; background:#fff; border:1px solid #ccc; box-shadow:0 0 10px #aaa;">
    <p id="confirmMessage">您是否要刪除資料？</p>
    <button onclick="proceedDelete()">是</button>
    <button onclick="closeDialog()">不是</button>
</div>
    <!-- 主資料區塊：置中對齊 -->
    <div align="center">
        <!-- 建立表格，顯示 RFID 卡號資料 -->
...
<script>
	...
	{
        deleteId = id;
        document.getElementById("confirmMessage").innerText = "您是否要刪除第 " + id + " 筆資料？";
	...
	{
        if (deleteId !== null) {
            window.location.href = "rfidDatadel.php?id=" + deleteId;
        }
    }
</script>