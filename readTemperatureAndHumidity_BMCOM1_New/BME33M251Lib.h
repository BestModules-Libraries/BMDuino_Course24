...
double temp ; //感測器溫度值之全域變數
double humid ; //感測器濕度值之全域變數

void initBME33M251() ; //初始化溫溼度感測器
float readTemperature();//讀取溫度感測器值
float readHumity() ; //讀取溼度感測器值
void readSensor() ;//讀取溫溼度感測器值
void printSensorData() ; //感測模組：列印感測器物件之感測資料
...
float readTemperature() //讀取溫度感測器值
{
  return BMht.readTemperature();//讀取溫度感測器值
}

float readHumidity()  //讀取溼度感測器值
{
 return BMht.readHumidity() ;//讀取溼度感測器值
}

void readSensor() //讀取溫溼度感測器值
{
  temp = readTemperature() ;  //讀取溫度，單位為攝氏度 
  humid = readHumidity() ;   // 讀取相對濕度，單位為百分比 
...
void printSensorData()  //感測模組：列印感測器物件之感測資料
{
  ...
  Serial.print(humid);     // 讀取並顯示目前濕度值（單位：%）
  Serial.print(" %    ");                // 顯示濕度單位符號與間隔

  Serial.print("Temperature : ");        // 顯示溫度資訊標籤
  Serial.print(temp);  // 讀取並顯示目前溫度值（單位：攝氏）
